...
function Utility.GetWeakestHero(radius)
  local npcBot = GetBot();
...
function Utility.PositionAlongLane(lane)
  ...
  if lane == nil then
    if GetTeam() == TEAM_RADIANT then
      return Utility.Locations["RadiantBase"];
    else
      return Utility.Locations["DireBase"];
    end
  end
...
function Utility.BOT_MODE_STRING( botMode )
  
  if botMode == BOT_MODE_NONE then
    return "BOT_MODE_NONE"
  end
  if botMode == BOT_MODE_LANING then
    return "BOT_MODE_LANING"
  end
  if botMode == BOT_MODE_ATTACK then
    return "BOT_MODE_ATTACK"
  end
  if botMode == BOT_MODE_ROAM then
    return "BOT_MODE_ROAM"
  end
  if botMode == BOT_MODE_RETREAT then
    return "BOT_MODE_RETREAT"
  end
  if botMode == BOT_MODE_SECRET_SHOP then
    return "BOT_MODE_SECRET_SHOP"
  end
  if botMode == BOT_MODE_SIDE_SHOP then
    return "BOT_MODE_SIDE_SHOP"
  end
  if botMode == BOT_MODE_PUSH_TOWER_TOP then
    return "BOT_MODE_PUSH_TOWER_TOP"
  end
  if botMode == BOT_MODE_PUSH_TOWER_MID then
    return "BOT_MODE_PUSH_TOWER_MID"
  end
  if botMode == BOT_MODE_PUSH_TOWER_BOT then
    return "BOT_MODE_PUSH_TOWER_BOT"
  end
  if botMode == BOT_MODE_DEFEND_TOWER_TOP then
    return "BOT_MODE_DEFEND_TOWER_TOP"
  end
  if botMode == BOT_MODE_DEFEND_TOWER_MID then
    return "BOT_MODE_DEFEND_TOWER_MID"
  end
  if botMode == BOT_MODE_DEFEND_TOWER_BOT then
    return "BOT_MODE_DEFEND_TOWER_BOT"
  end
  if botMode == BOT_MODE_ASSEMBLE then
    return "BOT_MODE_ASSEMBLE"
  end
  if botMode == BOT_MODE_TEAM_ROAM then
    return "BOT_MODE_TEAM_ROAM"
  end
  if botMode == BOT_MODE_FARM then
    return "BOT_MODE_FARM"
  end
  if botMode == BOT_MODE_DEFEND_ALLY then
    return "BOT_MODE_DEFEND_ALLY"
  end
  if botMode == BOT_MODE_EVASIVE_MANEUVERS then
    return "BOT_MODE_EVASIVE_MANEUVERS"
  end
  if botMode == BOT_MODE_ROSHAN then
    return "BOT_MODE_ROSHAN"
  end
  if botMode == BOT_MODE_ITEM then
    return "BOT_MODE_ITEM"
  end
  if botMode == BOT_MODE_WARD then
    return "BOT_MODE_WARD"
  end
  if botMode == 7 then
    return "BOT_MODE_RUNE"
  end
  return "BOT_MODE_UNKNOWN: "..botMode;
end
